...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_record') {
    ...
    // Validate mandatory fields (this depends on your specific requirements)
    $required_fields = [
        // 'date_of_apprehension' => $date_of_apprehension,
        // 'sitio' => $sitio,
        'barangay' => $barangay,
        'city_municipality' => $city_municipality,
        'province' => $province,
        'apprehending_officer' => $apprehending_officer,
        'apprehended_items' => $apprehended_items,
        'EMV_forest_product' => $EMV_forest_product,
        // 'EMV_conveyance_implements' => $EMV_conveyance_implements,
        'involve_personalities' => $involve_personalities,
        'custodian' => $custodian,
        ...
        // 'apprehended_vehicle_type' => $apprehended_vehicle_type,
        // 'apprehended_vehicle_plate_no' => $apprehended_vehicle_plate_no,
        // 'depository_sitio' => $depository_sitio,
        'depository_barangay' => $depository_barangay,
        'depository_city' => $depository_city,
        'depository_province' => $depository_province,
    ...
    foreach ($required_fields as $field => $value) {
        if (empty($value)) {
            $response = ['status' => 'error', 'message' => "The field '$field' is required and cannot be empty."];
    ...
    $sql = "INSERT INTO inventory (
        ...
        apprehended_vehicle_plate_no, date_created, create_by, linear_mtrs,species_type,species_status,
        depository_sitio, depository_barangay, depository_city, depository_province
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,?)";

    if ($stmt = $connection->prepare($sql)) {
        $stmt->bind_param('sssssssssssssssdsssssssssssss', 
