            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        //Call the logic of subtracting values from request_form to inventory table
                        //console.log(id); // Ensure id is defined correctly
                        $.ajax({
                            url: '/Admin/Requests/Approved/get-inventoryId.php',
                            type: 'GET',
                            contentType: 'application/json',
                            dataType: 'json',
                            data: {id: id}, // Ensure id is passed correctly
                            success: function(response) {
                                // console.log('response', response);
                                if (response && response[0]) {
                                    // console.log('ID:', response[0].id); 
                                    // console.log('Item ID:', response[0].item_id); 
                                    // console.log('Quantity:', response[0].quantity);

                                    var requestID=response[0].id;
                                    var inventoryId=response[0].item_id;
                                    var requestFormQuantity=response[0].quantity

                                    const updatetData = {
                                        requestID: requestID,
                                        inventoryId: inventoryId,
                                        requestFormQuantity: requestFormQuantity
                                    };

                                    //call pUT logic for updating quantity
                                    $.ajax({
                                        url: '/Admin/Requests/Approved/update-inventory-qty.php',
                                        type: 'PUT',
                                        contentType: 'application/json',
                                        dataType: 'json',
                                        data: JSON.stringify(updatetData), // Ensure id is passed correctly
                                        success: function(response) {
