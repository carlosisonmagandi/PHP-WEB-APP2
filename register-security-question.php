...
$querySecurityQuestion = "
    ...
    VALUES (?, ?, ?, ?, (SELECT id FROM account WHERE username = ? AND password = ?))
";

$stmt = mysqli_prepare($connection, $querySecurityQuestion);

if ($stmt) {    
    mysqli_stmt_bind_param($stmt, 'ssssss', $question1, $question2, $questionInput1, $questionInput2, $userName, $password);
